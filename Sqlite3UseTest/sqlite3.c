...
#define DATABASE "/nfs/MEI/Sqlite3UseTest/printer.db"	//数据库文件存放路径
#define RTC "/dev/rtc"
#define  SQL_LEN 256
sqlite3 *db = NULL;
char *zErrMsg = 0;
...
void RD_user_permission(int id)
{
	...
	*/
	...
	printf("per.print_per:%d ,per.scan_per:%d ,per.copy_per:%d\n",per.print_per,per.scan_per,per.copy_per);
	sqlite3_close(db);
...
void RD_AllKindsofPages(AllKindsofPages &akps)
{
	...
	//for(i= 0 ; i<(nrow +1)*ncolumn;i++)
		//printf("azResult[%d] = %s\n", i ,azResult[i]);
	//printf("azResult[4] %d",azResult[4]);
	akps.total = atoi(azResult[3]);
        akps.printed = atoi(azResult[4]);
        akps.duplicated = atoi(azResult[5]);
        akps.scaned = atoi(azResult[6]);
	//释放掉azResult的内存空间
	sqlite3_free_table(azResult);
	sqlite3_close(db);
...
void ReadPageSize(int &pageH ,int &pageW ,int &ncolormode ,int cate)
{
	...
	//for(i= 0 ; i<(nrow +1)*ncolumn;i++)
		//printf("azResult[%d] = %s\n", i ,azResult[i]);
	//printf("azResult[4] %d",azResult[4]);
	pageH = atoi(azResult[3]);
        pageW = atoi(azResult[4]);
        ncolormode = atoi(azResult[5]);
	//释放掉azResult的内存空间
	sqlite3_free_table(azResult);
	sqlite3_close(db);
...
void RD_WN(SetDateofWN &wn)
{
	...
	//for(i= 0 ; i<(nrow +1)*ncolumn;i++)
		//printf("azResult[%d] = %s\n", i ,azResult[i]);
	//printf("azResult[4] %d",azResult[4]);
	wn.tem1 = atoi(azResult[3]);
        wn.tem2 = atoi(azResult[4]);
        wn.tint = atoi(azResult[5]);
	//释放掉azResult的内存空间
	sqlite3_free_table(azResult);
	sqlite3_close(db);
}
